...
add_new_node(){
    ...
    colorize yellow "	Connecting to remote node on port 2200...\n" bold
    sshpass -p $password ssh -p 2200 -o ConnectTimeout=3 -o StrictHostKeyChecking=no $username@$host_ip "whoami" &> /dev/null
    ...
    #Transfer public key
	sshpass -p $password ssh-copy-id -i ~/.ssh/id_rsa.pub -p 2200 $username@$host_ip >/dev/null 2>&1 
...
ssh -p 2200 -T $username@$host_ip << EOF
TERM=xterm clear
...
#______________________ SSH CLOSED ____________________
	...
	ssh -p 2200 -T $username@$host_ip sudo mkdir /tmp/ &> /dev/null
	scp -P 2200 ${HOST_DIRECTORY}/main $username@$host_ip:/tmp/ &> /dev/null
	ssh -p 2200 -T $username@$host_ip sudo mv -v /tmp/main $HOST_DIRECTORY &> /dev/null

	# SSH into the remote server and read the contents of a file into a variable	
	echo -e "8. Exchanging node key to main server ..."
	node_with_key=$(ssh -p 2200 -T $username@$host_ip "cat ${HOST_DIRECTORY}/$node_name")
	echo "$node_with_key" > ${HOST_DIRECTORY}/$node_name 

	#start matrix service on node
	echo -e "9. Start Matrix serivce on the node server..."
	ssh -p 2200 -T $username@$host_ip "systemctl enable --now tinc@matrix" &> /dev/null
	# Maybe service exists before so restart it
	ssh -p 2200 -T $username@$host_ip "systemctl restart tinc@matrix" &> /dev/null
...
change_port_number(){
	...
	if [[ $1 == "main" ]];then
		...
		ssh -p 2200 -o BatchMode=yes -T "$ip_address" "sed -i 's/Port = [0-9]*/Port = $new_port/' \"$CONFIG_FILE\""   &> /dev/null
		ssh -p 2200 -o BatchMode=yes -T "$ip_address" "systemctl restart tinc@matrix"  &> /dev/null
	fi
	echo ''
...
remove_node(){
	...
	if [[ "$1" == "main" ]]; then
		...
		ssh -p 2200 -T -o BatchMode=yes -o ConnectTimeout=3 "$public_ip" << EOF
TERM=xterm clear
systemctl stop tinc@matrix &> /dev/null
systemctl disable tinc@matrix &> /dev/null
rm -rf "$CONFIG_DIRECTORY" &> /dev/null
...
restart_matrix_service(){
	...
	if [[ "$1" == "main" ]]; then
		...
		local CONFIG_FILE=${HOST_DIRECTORY}/${1}
		local public_ip=$(grep -oP '(?<=Address = )\S+' "$CONFIG_FILE")
		ssh -p 2200 -T -o BatchMode=yes -o ConnectTimeout=3 "$public_ip" "systemctl restart tinc@matrix" &> /dev/null
		colorize green "	Matrix service in $1 node restarted successfully \n" bold
		press_key
		return 0
	fi
...
reboot_node_server(){
	...
	if [[ "$1" == "main" ]]; then
		...
		ssh -p 2200 -T -o BatchMode=yes -o ConnectTimeout=3 "$public_ip" "reboot" &> /dev/null
...
tunnel_helper(){
	...
	if [ "$choice" == "2" ]; then
	    ...
	    local CONFIG_FILE_IRAN=${HOST_DIRECTORY}/$iran_node
  		...
  		ssh -p 2200 -T -o BatchMode=yes -o ConnectTimeout=3 "$public_ip_iran" "iptables -t nat -F" &> /dev/null
